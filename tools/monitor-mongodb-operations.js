...
function updateOperationStats(collectionName, operation, executionTime) {
  const opStats = stats[collectionName][operation];
  opStats.count++;
...
async function setupProfiler(db) {
  try {
...
async function checkRecentOperations(db) {
  try {
    ...
    operations.forEach(op => {
      ...
      if (Object.values(COLLECTIONS).includes(collectionName)) {
        ...
        if (stats[collectionName] && stats[collectionName][operationType]) {
...
async function checkTweetStorage(db) {
  try {
...
function displayStats() {
  ...
  Object.values(COLLECTIONS).forEach(collectionName => {
    const collStats = stats[collectionName];
    ...
    console.log(`${colors.dim}Size: ${(collStats.sizeBytes / 1024 / 1024).toFixed(2)} MB${colors.reset}`);
    
    console.log(`\n${colors.bright}Operations:${colors.reset}`);
    
    // Display operation stats
    Object.values(OPERATIONS).forEach(operation => {
      const opStats = collStats[operation];
