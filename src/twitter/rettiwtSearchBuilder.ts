...
export class RettiwtSearchBuilder {
  ...
  buildFilter(config: SearchQueryConfig): TweetFilter {
    ...
    try {
      ...
      const fromUsers = config.accounts?.map(a => a.replace(/^@/, '')) || [];
      ...
      const filter = {
        ...
        links: false,
        retweets: !config.excludeRetweets,
        quotes: !config.excludeQuotes,
        ...
        replies: config.advancedFilters?.include_replies ?? true,
        excludeWords: config.advancedFilters?.exclude_words,
        exactPhrases: config.advancedFilters?.exact_phrases
      };

      this.logger.debug(`Built filter from config: ${JSON.stringify(filter)}`);
  ...
  private validateConfig(config: SearchQueryConfig): FilterValidationResult {
    ...
    if (config.mentions?.length) {
      config.mentions.forEach((mention, index) => {
  ...
  private buildComplexSearchCriteria(config: SearchQueryConfig): string[] {
    const baseCriteria = config.keywords?.map(k => k.toLowerCase()) || [];
    
    if (!config.queryGroups?.length) {
      return baseCriteria;
    }

    const groupCriteria = config.queryGroups.map(group => 
  ...
  private validateAdvancedFilters(filters?: AdvancedFilter): FilterValidationResult {
    ...
    if (filters?.hashtags?.length) {
      filters.hashtags.forEach((tag, i) => {
        if (!/^#[\w]+$/.test(tag)) {
